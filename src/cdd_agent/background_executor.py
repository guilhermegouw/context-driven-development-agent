...
class BackgroundProcess:
    ...
    def __init__(
        ...
        logger.info(
            f"BackgroundProcess initialized: {process_id} for command: {command[:100]}"
        )
    ...
    def interrupt(self) -> bool:
        ...
        if self.process is None or not self.is_running():
            logger.warning(
                f"Attempted to interrupt non-running process: {self.process_id}"
            )
            return False
        ...
        try:
            ...
            self.output_queue.put(
                (self.process_id, "INTERRUPTED", "Process interrupted by user")
            )
    ...
    def _execute_process(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Subprocess started with PID {self.process.pid} for {self.process_id}"
            )
            ...
            if self.process and self.process.stdout:
                for line in iter(self.process.stdout.readline, ""):
                    if self._stop_event.is_set():
                        logger.info(
                            f"Stop event set, breaking loop for {self.process_id}"
                        )
                        break
                    ...
                    self.output_queue.put((self.process_id, "OUTPUT", line))
            ...
            try:
                ...
                logger.info(
                    f"Process {self.process_id} completed with exit code "
                    f"{self.exit_code}"
                )
                ...
                self.output_queue.put((self.process_id, "TIMEOUT", self.error_message))
            ...
            self.output_queue.put((self.process_id, "ERROR", self.error_message))
...
class BackgroundExecutor:
    ...
    def __init__(self, max_processes: int = 10):
        ...
        self.output_queue: Queue[Tuple[str, str, Union[str, dict]]] = queue.Queue()
    ...
    def execute_command(self, command: str, timeout: int = 300) -> BackgroundProcess:
        ...
        with self._lock:
            ...
            if active_count >= self.max_processes:
                raise ValueError(
                    f"Maximum number of background processes "
                    f"({self.max_processes}) exceeded"
                )
