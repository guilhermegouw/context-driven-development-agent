...
class TestAnthropicLazyLoading:
    ...
    def test_agent_creation_does_not_import_anthropic(self):
        ...
        try:
            ...
            with patch('cdd_agent.agent.ToolRegistry') as mock_registry, \
                 patch('cdd_agent.agent.ApprovalManager') as mock_approval, \
                 patch('cdd_agent.agent.ContextLoader') as mock_context:
                
                mock_registry.return_value.get_tool_schema.return_value = {"type": "function"}
                ...
                assert 'anthropic' not in sys.modules, "anthropic should not be imported on Agent creation"
                ...
                with patch('anthropic.Anthropic') as mock_anthropic:
                    ...
                    assert 'anthropic' in sys.modules, "anthropic should be imported when accessing client"
    ...
    def test_simple_agent_lazy_loading(self):
        ...
        try:
            ...
            assert 'anthropic' not in sys.modules, "anthropic should not be imported on SimpleAgent creation"
            ...
            with patch('anthropic.Anthropic') as mock_anthropic:
                ...
                assert 'anthropic' in sys.modules, "anthropic should be imported when accessing SimpleAgent client"
                mock_anthropic.assert_called_once_with(
                    api_key="test-key",
                    base_url="https://api.anthropic.com"
                )
    ...
    def test_cli_commands_do_not_import_providers_eagerly(self):
        ...
        try:
            ...
            assert 'anthropic' not in sys.modules, "anthropic should not be imported on CLI import"
            assert 'openai' not in sys.modules, "openai should not be imported on CLI import"
            assert 'cdd_agent.agent' not in sys.modules, "agent module should not be imported on CLI import"
...
class TestStartupPerformance:
    ...
    def test_startup_time_with_lazy_loading(self):
        ...
        try:
            ...
            assert startup_time < 0.1, f"CLI import took {startup_time:.3f}s, expected < 0.1s"
