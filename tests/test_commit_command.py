...
class TestCommitCommandStagedChanges(unittest.TestCase):
    ...
    def test_get_staged_diff_error(self, mock_run):
        """Test getting staged diff handles errors."""
        mock_run.side_effect = Exception("Git error")
        result = self.command._get_staged_diff()
...
class TestCommitCommandExecute:
    ...
    async def test_starts_commit_flow(self, mock_generate, mock_diff, mock_has_staged):
    ...
    async def test_abort_flag(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            assert "aborted" in result.lower() or "unstaged" in result.lower()

    @patch.object(CommitCommand, "_has_staged_changes")
    @patch.object(CommitCommand, "_get_staged_diff")
    @patch.object(CommitCommand, "_generate_commit_message")
    async def test_push_flag_sets_state(self, mock_generate, mock_diff, mock_has_staged):
...
@pytest.mark.asyncio
class TestCommitCommandUserResponse:
    """Test handling user responses in commit flow."""

    def setup_method(self):
        """Set up test fixtures."""
        self.command = CommitCommand()
        self.command._awaiting_response = True
        self.command._current_message = "feat: test commit"
        self.command._staged_changes = "diff content"
    ...
    async def test_accept_response(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            result = await self.command._handle_user_response("accept")

            assert "Successfully committed" in result
            assert self.command._awaiting_response is False
    ...
    async def test_yes_response(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            result = await self.command._handle_user_response("yes")

            assert "Successfully committed" in result

    async def test_abort_response(self):
        """Test aborting commit."""
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(returncode=0, stderr="")

            result = await self.command._handle_user_response("abort")

            assert "aborted" in result.lower()
            assert self.command._awaiting_response is False

    async def test_revision_response(self):
        ...
        with patch.object(
            ...
            result = await self.command._handle_user_response("use feat(api) type")

            assert "Proposed commit message" in result
            assert "feat(api): revised message" in result
            assert self.command._awaiting_response is True

    async def test_empty_response(self):
        """Test empty response prompts for valid input."""
        result = await self.command._handle_user_response("")

        assert "provide a valid response" in result.lower()
...
class TestCommitCommandState(unittest.TestCase):
    ...
    def test_initial_state(self):
        ...
        self.assertEqual(self.command._staged_changes, "")
        self.assertFalse(self.command._should_push)
        self.assertFalse(self.command._awaiting_response)
        self.assertIsNone(self.command._response_type)

    def test_reset_state(self):
        """Test state reset."""
        # Set some state
        self.command._current_message = "test"
        self.command._staged_changes = "diff"
        self.command._should_push = True
        self.command._awaiting_response = True
        self.command._response_type = "action"
