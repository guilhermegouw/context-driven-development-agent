...
class TestBackgroundTools(unittest.TestCase):
    ...
    def test_get_background_status_success(self):
        ...
        match = re.search(r'Background process started: (\w+-\w+-\w+-\w+-\w+)', start_result)
        ...
        if match:
            ...
            valid_statuses = ["running", "completed", "failed", "interrupted", "timeout"]
            status_match = any(status_val in status for status_val in valid_statuses)
            self.assertTrue(status_match, f"Status should contain one of {valid_statuses}")
    ...
    def test_interrupt_background_process_success(self):
        ...
        start_result = run_bash_background("for i in {1..10}; do echo 'Step $i'; sleep 0.1; done")
        match = re.search(r'Background process started: (\w+-\w+-\w+-\w+-\w+)', start_result)
    ...
    def test_integration_complete_workflow(self):
        ...
        start_result = run_bash_background("for i in {1..3}; do echo \"Step $i\"; sleep 0.2; done")
        match = re.search(r'Background process started: (\w+-\w+-\w+-\w+-\w+)', start_result)
        self.assertIsNotNone(match)
        process_id = match.group(1)
        ...
        self.assertTrue(
            "Step 1" in output or "No output available" in output
        )
